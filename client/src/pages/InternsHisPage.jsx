...
const InternsPage = () => {
  ...
  // Filter and sort interns by updatedAt
  const filteredInterns = interns
    ...
    .sort((a, b) => {
      ...
      return bDate - aDate; // Sort by most recent mark (descending)
    });
  ...
  const handleMarkAttendance = async (id, status) => {
    ...
    try {
      const response = await api.post(
        `/interns/mark-attendance`,
        { internId: id, status, date: selectedDate },
        getAuthHeaders()
      );

      if (response.status === 200) {
        // Update the intern's data in state with the new attendance status and updatedAt
          ...
          i._id === id
            ? {
                ...
                attendance: [
                  ...
                  { date: selectedDate, status, timestamp: new Date() },
                ],
  ...
  const generatePDF = () => {
    try {
      ...
      // Prepare table data
      const tableData = filteredInterns
        .map((intern) => {
          const attendanceForDate = (intern.attendance || []).find((entry) => {
            try {
              const entryDate = new Date(entry.date).toLocaleDateString();
      ...
      let sortingInfo = `Sorted by: Most Recent Attendance`;
      if (searchTerm) sortingInfo += ` • Search: "${searchTerm}"`;
      if (selectedSpecialization)
        sortingInfo += ` • Specialization: "${selectedSpecialization}"`;
      ...
      >
        ...
        {!loading && (
          <>
            ...
            {interns.some((i) => i.attendanceStatus !== "Not Marked") && (
              <div className="mt-8 bg-white p-6 rounded-lg shadow-md">
                ...
                <div className="overflow-x-auto">
                  <table className="w-full border-collapse">
                    ...
                    <tbody>
                      {interns
                        .filter((i) => i.attendanceStatus !== "Not Marked")
                        .sort((a, b) => {
                          ...
                          const bDate = b.updatedAt
                            ...
                            : new Date(0);
                          return bDate - aDate; // Sort by most recent mark
                        })
                        .map((intern) => {
                          const attendanceEntry = intern.attendance?.find(
                            (entry) =>
                              new Date(entry.date).toDateString() ===
                              new Date(selectedDate).toDateString()
                          );
                          ...
                          return (
                            ...
                            >
                              <td className="p-3">{intern.traineeId}</td>
                              <td className="p-3">{intern.traineeName}</td>
                              <td className="p-3">
                                <span
