...
import InternSelectionModal from "../components/InternSelectionModal";
...
const InternsPage = () => {
  ...
  const handleMarkAttendance = async (id, status) => {
    ...
    try {
      ...
      toast.error(`Error marking attendance: ${errorMessage}. Make sure you are logged in.`);
    }
  };
    ...
    <Layout className="relative">
      ...
      >
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-4 md:mb-6 gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="bg-purple-600 text-white px-4 py-2.5 rounded-lg flex items-center gap-2 hover:bg-purple-700 transition-all duration-200 hover:shadow-md"
            >
              <User size={18} />
              <span className="font-medium">Select & Mark</span>
            </button>
          </div>
        </div>
        ...
        <InternSelectionModal
          ...
          interns={interns}
          selectedInterns={selectedInternIds}
          onSelectInterns={(ids) => setSelectedInternIds(ids)}
          onMarkAttendance={async (internId, status, date) => {
            try {
              await api.post(
                `/interns/mark-attendance`,
                { internId, status, date },
                getAuthHeaders()
              );
              ...
              toast.error(`Failed to mark attendance for ${internId}`);
              return false;
            }
          }}
                  ...
                  {currentInterns.length > 0 ? (
                    currentInterns.map((intern) => (
                      ...
                      >
                        ...
                        <td className="p-4 border-b">{intern.institute || ""}</td>
